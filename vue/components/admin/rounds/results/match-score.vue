        ...
        computed: {
            ...
            rowData() {
                ...
                for (let i = 0; i < this.round.questions; i++) {
                    ...
                    const question = this.questions.filter(q => q.matchid === this.match.id && q.number === i + 1);
            ...
            leftLoser() {
                return this.match.winner_mdl_user > 0 && this.match.winner_mdl_user !== this.userLeft.id;
            },
            rightLoser() {
                return this.match.winner_mdl_user > 0 && this.match.winner_mdl_user !== this.userRight.id;
            }
        },
        methods: {
            getAttempt(question, user) {
                const attempts = this.attempts.filter(a => a.question === question.id && a.mdl_user === user.id);
            ...
            getScoreSum(user) {
                const questionIds = this.questions.filter(q => q.matchid === this.match.id).map(q => q.id);
                const attemptScores = this.attempts.filter(a => questionIds.includes(a.question) && a.mdl_user === user.id).map(a => a.score);

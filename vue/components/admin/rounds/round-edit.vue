...
export default {
  ...
  computed: {
    ...
    roundCategories() {
      ...
      return this.categories.filter((category) => {
        ...
        const lastRound =
          category.round_last === 0
            ? null
            : this.getRoundById(category.round_last);
        return (
          firstRound.number <= this.data.number &&
          (lastRound === null ? true : lastRound.number >= this.data.number)
        );
      });
    },
    activeCategories() {
      const categories = this.roundCategories.filter((category) => {
        return (
          this.categoryChanges.deleted.findIndex(
            (c) => c.id === category.id
          ) === -1
        );
      });
  ...
  methods: {
    ...
    getCategoryLabel(category, index) {
      if (category.id === null) {
        return this.stringParams(
          this.strings.admin_round_lbl_category_new,
          index + 1
        );
      }
      const roundFirst = this.getRoundById(category.round_first);
      if (category.round_last === 0) {
        return this.stringParams(this.strings.admin_round_lbl_category_open, {
          number: index + 1,
      ...
      if (category.round_first === category.round_last) {
        return this.stringParams(
          this.strings.admin_round_lbl_category_closed_same,
          { number: index + 1, round_first_number: roundFirst.number }
        );
      }
      const roundLast = this.getRoundById(category.round_last);
      return this.stringParams(
        this.strings.admin_round_lbl_category_closed_range,
        {
          number: index + 1,
          round_first_number: roundFirst.number,
    ...
    removeCategory(category) {
      if (category.id) {
        this.categoryChanges.deleted = concat(this.categoryChanges.deleted, [
          category,
        ]);
      } else {
        this.categoryChanges.added = this.categoryChanges.added.filter(
          (c) => c !== category
        );
      }
    },
    ...
    save() {
      const addedCategories = map(
        this.categoryChanges.added,
        transformCategoryToDTO
      );
      const deletedCategories = map(
        this.categoryChanges.deleted,
        transformCategoryToDTO
      );
      let result = {
      ...
      this.saving = true;
      this.saveRound(result).then((successful) => {
        this.saving = false;
        if (successful) {
          this.goToRoundList();
        }

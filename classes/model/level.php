...
class level extends abstract_model {
    ...
    function __construct() {
        parent::__construct('challenge_levels', 0);
        $this->game = 0;
    ...
    public function get_categories(): array {
        ...
        $records = $DB->get_records('challenge_categories', $sql_params);
    ...
    public function delete_image($context) {
        if ($this->get_image()) {
            $fs = get_file_storage();
            if ($file = $fs->get_file($context->id, 'mod_challenge', self::FILE_AREA, $this->get_id(), '/', $this->get_image())) {
                $file->delete();
            }
            $this->set_image('');
        }
